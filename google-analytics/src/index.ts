...
export default new botpress.Integration({
  register: async ({ ctx }) => {
    ...
    try {
      ...
      throw new bpclient.RuntimeError(
        'Configuration Error! Invalid Google Analytics Measurement ID or API Secret.'
      )
    }
  },
  unregister: async () => {},
  actions: {
    trackNode: async (args) => {
      ...
      try {
        await axios.post(`https://www.google-analytics.com/mp/collect?measurement_id=${args.ctx.configuration.measurementId}&api_secret=${args.ctx.configuration.apiSecret}`, {
